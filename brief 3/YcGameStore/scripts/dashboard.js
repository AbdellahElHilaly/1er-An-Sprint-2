...
function edit(event) {
    ...
    for(i=2 ; i<6 ; i++){
        ...
        if(i==4 || i==5) elt.children[i].innerHTML = "<input class='change-input' type='number' value='" + value + "'>"
        else elt.children[i].innerHTML = "<input class='change-input' type='text' value='" + value + "'>"
        tempGame[i] = value;
    }
    ...
    elt.children[3].innerHTML =  
        `
            <select class="change-input w-100" aria-label="Default select example">
                ...
                <option value="1">shoter</option>
                <option value="2">patel royal</option>
...
function saveUpdate(event) {
    ...
    // trensfetr category from string to number 
    if(tempGame[3] == "shoter") tempGame[3] = 1;
    if(tempGame[3] == "patel royal") tempGame[3] = 2;
    if(tempGame[3] == "puzzle") tempGame[3] = 3;
    if(tempGame[3] == "sport") tempGame[3] = 4;
    if(tempGame[3] == "simulation") tempGame[3] = 5;
    ...
    $.post('../scripts/script.php' ,
        ...
        function(data, status) {// success callback
            ...
            elt.children[3].innerHTML = data.category;
...
*/
function Search(){
    if(getSearchMessage() == "") displayGames(0);
    else initialazeTabelGames();
    
     // AJAX send data from  this to script.php
    $.post('../scripts/script.php' ,
    {
        request:"Search",
        indis:getSearchIndis(),
...
function getSearchIndis(){
    let seachrByName = document.getElementById('check-name');
    let seachrByCategory = document.getElementById('check-category');
    let seachrByPrice = document.getElementById('check-price');
    let seachrQuantity = document.getElementById('check-quantity');
    let seachrId = document.getElementById('check-id');


    if(seachrByName.checked == true)  return 1;
    if(seachrByCategory.checked == true)  return 2;
    if(seachrByPrice.checked == true)  return 3;
    if(seachrQuantity.checked == true)  return 4;
    if(seachrId.checked == true)  return 5;
